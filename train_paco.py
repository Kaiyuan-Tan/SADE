...
from trainer import PaCoTrainer
import pdb
deterministic = False
if deterministic:
...
def main(config):
    ...
    # pdb.set_trace()
    if config.resume is not None and lr_scheduler is not None:
        print("Reset LR scheduler for resume training")
        lr_scheduler.last_epoch = 0
        base_lr = config['optimizer']['args']['lr']
        print(f"Force reset optimizer LR to {base_lr}")

        for g in optimizer.param_groups:
            g['lr'] = base_lr

    trainer = PaCoTrainer(model, criterion, metrics, optimizer,
                      config=config,
