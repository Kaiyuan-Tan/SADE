        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            with autocast():
                if self.real_model.requires_target:
                    ...
                    if self.add_extra_info:
                        ce_loss, supcon_loss = self.criterion(output_logits=output, target=target, extra_info=extra_info)
                        loss = ce_loss + supcon_loss
                    else:
                        loss = self.criterion(output_logits=output, target=target)
                        ce_loss = loss
                        supcon_loss = torch.tensor(0.0).to(self.device)
            if not use_fp16:
                loss.backward()
                self.optimizer.step()
            else:
